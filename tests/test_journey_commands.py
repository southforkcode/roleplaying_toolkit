...
class TestJourneyCommandIntegration:
    ...
    def test_progress_command_integration(self):
        ...
        cmd = MockCommand(["2"])
        manager = JourneyManager()
        manager.start_journey("Test Quest", 5, "medium")
    ...
    def test_status_with_journeys_integration(self):
        ...
        cmd = MockCommand([])
        manager = JourneyManager()
        manager.start_journey("Quest One", 5, "medium")
        manager.start_journey("Quest Two", 3, "easy")
    ...
    def test_full_journey_workflow_via_handler(self):
        ...
        # Start a journey
        result1 = handler.process_input('journey "Epic Quest" 3 hard')
    ...
    def test_multiple_journey_stack_via_handler(self):
        ...
        # Start first journey
        result1 = handler.process_input('journey "Bottom Quest" 5 easy')
        assert result1["success"]
        
        # Start second journey (should become current)
        result2 = handler.process_input('journey "Top Quest" 2 medium')
        assert result2["success"]
    ...
    def test_journey_error_handling_via_handler(self):
        ...
        result3 = handler.process_input('journey "Bad Quest" 0 easy')
        assert not result3["success"]
        assert "positive number" in result3["message"]
        
        result4 = handler.process_input('journey "Bad Quest" 5 impossible')
        assert not result4["success"]
        assert "Difficulty must be one of:" in result4["message"]